    ...
    >
      <div className="container mx-auto px-4 flex items-center justify-between">
        <motion.a href="#home" className="flex items-center gap-3" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          ...
          <span className="font-display text-xl font-bold bg-gradient-to-r from-emerald-400 to-green-600 bg-clip-text text-transparent">
            ONE BLOCK
          </span>
        </motion.a>
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link, index) => (
            ...
            >
              {link.name}
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-emerald-400 to-green-600 transition-all duration-300 group-hover:w-full" />
            </motion.a>
          ))}
        </div>
